...
const App: React.FC = () => {
  ...
  const handlePlayRange = useCallback((surah: Surah, mode: 'verse-by-verse' | 'full-surah', isContinuation = false) => {
    ...
    if (mode === 'full-surah') {
        ...
        const combinedText = ayahsInRange.map(a => a.text.replace(/[۞۩]/g, '').trim()).join(' ');
        
        const fakeAyah: Ayah = { id: playbackRange.start, text: combinedText, translation: ayahsInRange[0].translation };
        const cacheKey = `${selectedReciterId}-${surah.id}-${playbackRange.start}-${playbackRange.end}`;
        ...
        if (firstAyahInRange) {
            const cacheKey = `${selectedReciterId}-${surah.id}-${firstAyahInRange.id}`;
            handlePlayAyah(firstAyahInRange, surah, mode, cacheKey, isContinuation);
        }
    }
  ...
  const handlePlayPause = useCallback(() => {
    ...
    if (audioContext.state === 'running' && playingState.status === 'playing') {
      ...
      setPlayingState(prev => ({ ...prev, status: 'paused' } as PlayingState));
      ...
      startElapsedTimer();
      setPlayingState(prev => ({ ...prev, status: 'playing' } as PlayingState));
    }
  ...
  const handleSeek = (time: number) => {
    if (playingState.mode !== 'full-surah' || !audioBufferRef.current || !audioContextRef.current) return;
